...
void RK4::integrateCluster(Cluster &system, double dt, int n_steps)
{
    ...
    std::vector<vec3> A  = std::vector<vec3>(2*n_bodies);
    std::vector<vec3> K1 = std::vector<vec3>(2*n_bodies);
    std::vector<vec3> K2 = std::vector<vec3>(2*n_bodies);
...
std::vector<vec3> RK4::dAdt(Cluster &system, std::vector<vec3> A)           // (24 * n(n+1)/2) + 3n= 12n^2 +15n FLOPs
{
    ...
    double G = 4*pi*pi;
    int n_bodies = system.numberOfBodies();
    std::vector<vec3> dAdt = std::vector<vec3>(2*n_bodies);
...
void RK4::integrateClusterAdaptive(Cluster &system, double dt, int n_steps)
{
    // Creating vectors
    int n_bodies = system.numberOfBodies();
...
std::vector<vec3> RK4::dAdtAdaptive(Cluster &system, std::vector<vec3> A)           // (24 * n(n+1)/2) + 3n= 12n^2 +15n FLOPs
{
    double pi = 4*std::atan(1.0);
    double G = 4*pi*pi;
    int n_bodies = system.numberOfBodies();
